...
for i,proxy in enumerate(proxies):
 ...
 f'sleep {i} && source ./env/bin/activate && yt-dlp -f 139 --proxy "{proxy}"  --socket-timeout 60 --retries 20  --output "/dev/null" "{url}" > /dev/null && [[ "$?" == "0" ]]   &&  echo {proxy} >> prooven_proxies_yt.dat'
 )
